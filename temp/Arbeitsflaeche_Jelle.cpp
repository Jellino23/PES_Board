...
// Globale Variablen
bool do_execute_main_task = false; // Steuert die AusfÃ¼hrung der Hauptaufgabe (wird durch Benutzerbutton getoggelt)
bool do_reset_all_once = false;    // Wird verwendet, um Variablen einmalig zurÃ¼ckzusetzen

// Objekte fÃ¼r Benutzerbutton (blauer Button) auf dem Nucleo-Board
...
// Hauptprogramm lÃ¤uft als eigener Thread
int main()
{
    // ZustÃ¤nde fÃ¼r die Zustandsmaschine
    enum RobotState {
        INITIAL,          // Initialisierungszustand
        PLATFORM,         // Plattform-Phase
        ROPEPREPARE,      // Vorbereitung fÃ¼r Seil-Phase
        ROPE,             // Seil-Phase
        OBSTACLEPREPARE,  // Vorbereitung fÃ¼r Hindernis-Phase
    ...
    // Haupttask wird alle main_task_period_ms Millisekunden ausgefÃ¼hrt
    const int main_task_period_ms = 20; // Haupttask-Periodendauer in ms (50 Hz)
    Timer main_task_timer;              // Timer fÃ¼r die periodische AusfÃ¼hrung
    ...
    // Kalibrierungswerte fÃ¼r die Servos (minimale/maximale Pulsbreite)
    ...
    int servo_counter_left = 0;  // ZÃ¤hler fÃ¼r Servobewegung
    int servo_counter_right = 0;
    const int loops_per_seconds = static_cast<int>(ceilf(1.0f / (0.001f * static_cast<float>(main_task_period_ms))));

    // Positionen fÃ¼r die Gewichte (Servo-Winkel)
    ...
    const float gear_ratio_M1 = 100.0f; // ÃœbersetzungsverhÃ¤ltnis
    ...
    float diff_Rot_M1_end = 0.0f; // Rotationsdifferenz fÃ¼r Endphase
    float diff_Rot_M2_end = 0.0f;
    
    int endplat_get_rotation = 1; // Flag fÃ¼r Rotationsmessung auf Endplattform

    int dance_servo_down = 0; // Status fÃ¼r Servos im Tanz
    int finish_turn = 1;      // Flag fÃ¼r Abschlussdrehung
    int finish_get_rotation = 1; // Flag fÃ¼r Rotationsmessung im Tanz
    ...
    // Reglerparameter fÃ¼r LineFollower
    ...
    float distance_to_ground = 10.0f; // Schwellwert fÃ¼r Bodenerkennung [cm]
    ...
    // Timer fÃ¼r verschiedene ZustÃ¤nde
    ...
    while (true) {
        ...
        if (do_execute_main_task) {
            // Visuelles Feedback, dass Haupttask lÃ¤uft
            led1 = 1;
            ...
            // Ultraschallsensor auslesen (nur gÃ¼ltige Werte Ã¼bernehmen)
            ...
            switch (robot_state) {
                case RobotState::INITIAL: {
                    ...
                    // Timer fÃ¼r Initialisierungsphase starten
                    ...
                    // Nach 3 Sekunden in nÃ¤chsten Zustand wechseln
                ...
                case RobotState::ROPEPREPARE: {
                    ...
                    // Servos langsam in Position bringen (rechts runter, links hoch)
                ...
                case RobotState::OBSTACLEPREPARE: {
                    ...
                    // Servos langsam in Ausgangsposition bringen (links runter, rechts hoch)
                    ...
                    if(servo_input_right > weight_up_right && servo_input_left < weight_up_left) {
                        ...
                        // Nach 0.75 Sekunden zurÃ¼ck zur Plattformphase
                ...
                case RobotState::DANCE: {
                    ...
                    if(finish_turn == 1) {
                        ...
                        // Drehung (M1 rÃ¼ckwÃ¤rts, M2 vorwÃ¤rts)
                        motor_M1.setVelocity(motor_M1.getMaxVelocity() * (-0.7));
                        motor_M2.setVelocity(motor_M2.getMaxVelocity() * 0.7);
                        diff_Rot_M1 = (motor_M1.getRotation() - rotationBeforeRopeM1);
                        diff_Rot_M2 = (motor_M2.getRotation() - rotationBeforeRopeM2);
                    ...
                    if(dance_servo_down == 1) {
                        ...
                        // Wenn Servos in Position: nÃ¤chste Phase
                    ...
                    // Servos zurÃ¼ck nach oben bewegen
            ...
            // Reset-Routine (wird einmal ausgefÃ¼hrt, wenn Haupttask deaktiviert wird)
        ...
        // Restliche Zeit bis zum nÃ¤chsten Zyklus warten
